...
public static class Recursion
{
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        ...
        for (int i = 0; i < letters.Length; i++)
        {
            // Choose letter at i
            char c = letters[i];
            // Recurse without the chosen letter (remove i-th char)
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        if (s < 0)
            return 0;
        if (s == 0)
            return 1;  // One way to stand still at bottom
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        if (maze.IsEnd(x, y))
        {
            results.Add(currPath.AsString()); // <-- FIXED: Uses AsString() to match test expectations
            return;
        }

        // Recurse in all four directions with a copy of the current path
